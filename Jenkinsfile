            ...
            steps {
                ...
                withCredentials([usernamePassword(credentialsId: 'myprod_server', usernameVariable: 'USERNAME', passwordVariable: 'USERPASS')]) {
                    script {
                        ...
                        sh "sshpass -p '$USERPASS' -v ssh -o StrictHostKeyChecking=no $USERNAME@$prod_ip \"docker run --restart always --name mymeanr -p 8080:8080 -d tester99/mymeanr:${env.BUILD_NUMBER}\""
                    }
                }
            }
        }
